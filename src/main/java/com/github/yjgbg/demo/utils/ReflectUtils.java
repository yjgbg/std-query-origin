package com.github.yjgbg.demo.utils;
...
public class ReflectUtils {
	...
	private Object getValue0(Object obj, String fieldName) {
		...
		final var getMethodName  = "get"+CaseFormat.LOWER_CAMEL.to(CaseFormat.UPPER_CAMEL,fieldName);
	...
	private void setValue0(Object obj,String fieldName,Object value) {
		...
		final var setMethodName = "set" + CaseFormat.LOWER_CAMEL.to(CaseFormat.UPPER_CAMEL, fieldName);
		final var method = Arrays.stream(obj.getClass().getMethods())
	...
	public void setValue(@Nullable Object obj, String path, Object value) {
		...
		if (obj instanceof Iterable) {
			((Iterable<?>) obj).forEach(item -> setValue(item, path, value));
			return;
		}
		final var arr = path.split("\\.");
		final var isMap = obj instanceof Map;
		final var lenEq1 = arr.length == 1;
		...
		if (isMap && lenEq1) {
			...
			final var subObj = ((Map<?, ?>) obj).get(arr[0]);
			final var subPath = path.replace(arr[0]+".","");
			setValue(subObj,subPath,value);
		} else if (lenEq1) {
			// 如果以上情况都不满足，那么set Value
			setValue0(obj,path,value);
		} else {// 如果存在点，那么get到值之后递归该函数
			final var subObj = getValue0(obj,arr[0]);
			final var subPath = path.replace(arr[0]+".","");
			setValue(subObj,subPath,value);
		}
	}
